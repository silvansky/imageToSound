...
def render_static(spec_db, kwargs, out_path, title, cmap, db_floor):
...
def encode_scroll_video(wide_png, wav, out_path, duration, pps, video_w, video_h):
    cmd = [
        "ffmpeg", "-y", "-loop", "1", "-framerate", "30",
        "-i", wide_png, "-i", wav,
        "-filter_complex",
        f"[0:v]pad=iw+{video_w}:ih:{video_w}:0:color=black,"
        f"crop={video_w}:{video_h}:{pps}*t:0,format=yuv420p[v]",
        "-map", "[v]", "-map", "1:a",
        "-c:v", "libx264", "-c:a", "aac",
...
def make_scroll(spec_db, wav, base, suffix, duration, pps, video_w, video_h, cmap_name, db_floor):
    ...
    encode_scroll_video(wide_png, wav, out_mkv, duration, pps, video_w, video_h)
    os.remove(wide_png)
